...
8. **行動優先與嚴謹設計系統（Mobile-First & Design Tokens）**：
   - **6 級排版階層規範（Typographic Scale）**：嚴格劃分主標、區塊標、卡片標、內文、輔助說明與膠囊標籤，杜絕任意像素與字體忽大忽小，全站最低字級底線為 `text-[11px]` 徽章，保證手機單手操作高可讀性。
   ...
   - **單手舒適操作**：專為單手操作設計的底部導覽列（含 iOS 安全邊界 `pb-safe`）、頂層 Portal 模態抽屜與清楚的生命週期進度條。
