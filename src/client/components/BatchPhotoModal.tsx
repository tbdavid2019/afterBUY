      ...
      <div className="app-surface w-full max-w-xl max-h-[90vh] rounded-3xl border border-[var(--app-border)] shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between p-4 sm:p-5 border-b border-[var(--app-border)]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="ui-section-title text-[var(--app-text)] tracking-tight">{t('batchPhotoUpload')}</h2>
              <p className="ui-meta text-[var(--app-muted)]">{t('batchPhotoDesc')}</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
          ...
          {drafts.length === 0 ? (
            <div className="space-y-3">
              <div className="text-center py-2">
                <p className="ui-label font-bold text-[var(--app-accent-strong)]">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <span className="ui-body font-bold text-[var(--app-text)]">
                    {locale === 'zh-TW' ? '拍照連續上傳' : 'Capture with Camera'}
                  </span>
                  <span className="ui-meta text-[var(--app-muted)] mt-1">
            ...
            <div className="flex items-center justify-between bg-[var(--app-surface-subtle)] border border-[var(--app-border)] rounded-2xl p-3">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-xl bg-[var(--app-accent-soft)] text-[var(--app-accent-strong)] flex items-center justify-center font-bold ui-label border border-[var(--app-border)]">
                  {drafts.length}
                </div>
                <div>
                  <p className="ui-body font-bold text-[var(--app-text)]">
              ...
              <div className="flex gap-1.5">
                <button
                  ...
                  disabled={uploading}
                  onClick={() => cameraInputRef.current?.click()}
                  className="app-control flex items-center gap-1.5 px-3 py-2 rounded-xl ui-meta font-semibold text-[var(--app-text)] active:scale-95 transition-all min-h-11"
                >
          ...
          {uploading && (
            <div className="flex items-center justify-center gap-2 p-4 bg-[var(--app-surface-subtle)] border border-[var(--app-border)] rounded-2xl ui-body text-[var(--app-accent-strong)]">
          ...
          {errorMessage && (
            <div className="ui-body text-rose-500 bg-rose-500/10 border border-rose-500/25 rounded-xl p-3.5">
              {errorMessage}
            </div>
          )}
          ...
          {drafts.length > 0 && (
            <div className="space-y-3 pt-1">
              <div className="flex items-center justify-between ui-meta text-[var(--app-muted)] px-1">
                <span>{locale === 'zh-TW' ? '步驟 2：確認各項物品名稱與週期' : 'Step 2: Review details & cycle'}</span>
                <span className="ui-meta text-[var(--app-accent-strong)] font-semibold">
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="w-20 h-20 rounded-xl border border-[var(--app-border)] shrink-0 bg-[var(--app-surface)] overflow-hidden shadow-inner relative">
                        ...
                        />
                        <span className="absolute bottom-1 right-1 bg-black/70 ui-badge font-bold text-white px-2 py-0.5 rounded">
                          #{idx + 1}
                        </span>
                      </div>
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        <input
                          ...
                          className="w-full bg-[var(--app-bg)] border border-[var(--app-border)] focus:border-[var(--app-accent)] rounded-xl px-3 py-2.5 text-[var(--app-text)] font-bold ui-body outline-none placeholder:text-[var(--app-muted-low)]"
                        />
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <select
                            ...
                            className="bg-[var(--app-bg)] border border-[var(--app-border)] focus:border-[var(--app-accent)] rounded-xl px-2.5 py-2 text-[var(--app-text)] ui-meta outline-none"
                          >
                          ...
                          <div className="flex items-center bg-[var(--app-bg)] border border-[var(--app-border)] rounded-xl px-2.5 py-2">
                            <input
                              type="number"
                              value={draft.cycleDays}
                              onChange={(e) => handleUpdateDraft(draft.id, 'cycleDays', parseInt(e.target.value) || 90)}
                              placeholder="週期天數"
                              className="w-full bg-transparent text-[var(--app-text)] font-semibold outline-none ui-meta"
                            />
                            <span className="ui-meta text-[var(--app-muted)] shrink-0">天</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className="text-[var(--app-muted)] hover:text-rose-500 p-2 transition-colors min-h-11 min-w-11 flex items-center justify-center"
                    ...
                    <div className="pt-1 border-t border-[var(--app-border)]">
                      <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
                        <span className="ui-meta text-[var(--app-muted)] shrink-0 font-medium mr-0.5">常用範本:</span>
                        {ITEM_PRESETS.slice(0, 8).map((p, pIdx) => (
                          <button
                            key={pIdx}
                            type="button"
                            onClick={() => handleApplyPresetToDraft(draft.id, p)}
                            className="ui-meta font-medium px-3 py-1.5 rounded-full border border-[var(--app-border)] hover:border-[var(--app-accent)] bg-[var(--app-surface)] text-[var(--app-muted)] hover:text-[var(--app-text)] shrink-0 active:scale-95 transition-all"
                          >
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-0.5">
                      <div className="flex items-center bg-[var(--app-bg)] border border-[var(--app-border)] rounded-xl px-2.5 py-2">
                        <span className="ui-meta text-[var(--app-muted)] mr-1">NT$</span>
                        <input
                          type="number"
                          value={draft.price}
                          onChange={(e) => handleUpdateDraft(draft.id, 'price', e.target.value === '' ? '' : parseInt(e.target.value) || 0)}
                          placeholder="購買金額 (選填)"
                          className="w-full bg-transparent text-[var(--app-text)] outline-none ui-meta placeholder:text-[var(--app-muted-low)]"
                        />
                      </div>
                      <input
                        type="text"
                        ...
                        placeholder="規格型號 (選填)"
                        className="w-full bg-[var(--app-bg)] border border-[var(--app-border)] rounded-xl px-2.5 py-2 text-[var(--app-text)] outline-none ui-meta placeholder:text-[var(--app-muted-low)]"
                      />
                    </div>
                  </div>
                ))}
          ...
          <div className="p-4 sm:p-5 border-t border-[var(--app-border)] flex items-center justify-between gap-3 bg-[var(--app-surface)] backdrop-blur-md">
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => setDrafts([])}
                className="app-control min-h-11 px-4 py-2.5 rounded-xl ui-label font-semibold text-[var(--app-muted)] hover:text-[var(--app-text)] transition-colors"
              >
                {t('cancel')}
              </button>
              <button
                ...
                className="app-primary min-h-11 px-5 py-2.5 rounded-xl ui-label font-bold flex items-center gap-1.5 shadow-sm active:scale-95 transition-all"
              >
